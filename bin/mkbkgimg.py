...
if __name__ == '__main__':
    ...
    if control_params.effect.ace == 'gauss':
        if acex_std != acey_std:
            print(
                'In the current gauss-ace mode, acex_std must be equal to acey_std. Sorry!')
            exit(-1)
        else:
    ...
    if True:
        ...
        for iplate in tqdm.tqdm(range(0, control_params.nplate)):
            ...
            if control_params.effect.ace != 'real':
                ...
                pixar = upixar[:, :, np.newaxis] + \
                    np.zeros((nxt, nyt, Nts_per_plate))
            ...
            integrated = integrate(
                pixar, x0_global, y0_global, tplate, dtace, detector)
