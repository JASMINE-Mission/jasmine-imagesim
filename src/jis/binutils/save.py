...
def save_outputs(filenames, output_format, control_params, telescope, detector, wfe, psf, pixcube_global, tplate, uniform_flat_interpix, uniform_flat_intrapix, acex, acey, overwrite):
    ...
    if control_params.effect.flat_interpix is True:
        pf.writeto(filenames["interpix"], detector.flat.interpix, overwrite=overwrite)
    else:
        pf.writeto(filenames["interpix"], uniform_flat_interpix, overwrite=overwrite)
    if control_params.effect.flat_interpix is True:
        pf.writeto(filenames["intrapix"], detector.flat.intrapix, overwrite=overwrite)
    else:
        pf.writeto(filenames["intrapix"], uniform_flat_intrapix, overwrite=overwrite)
    ...
    if output_format == 'hdfcube':
        ...
        if output_format == 'platefits':
            for i in range(0, control_params.nplate):
                pf.writeto(filenames["images"][i], pixcube_global[i].astype('int32'), overwrite=overwrite)
        elif output_format == 'fitscube':
            pf.writeto(filenames["images"][0], pixcube_global.astype('int32'), overwrite=overwrite)
